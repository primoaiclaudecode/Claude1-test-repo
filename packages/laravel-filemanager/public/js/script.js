...
$(document).on('click', '.file-item', function (e) {
  //useFile($(this).data('id'));
	...
	var fileURL 	= $(this).data('id');
	var file_idVal	= $('#file_id').val();
	
	var resSET 		= fileURL.replace("https://sam6.primosolutions.ie/laravel-filemanager/", "");
	var aLink  		='<span><a href="'+fileURL+'" target="_blank">'+fileTitle+'</a><a href="javascript:void(0);" class="del-file" style="float:right;" data-fid="'+resSET+'"><i class="fa fa-trash fa-fw"></i></a><br></span>';
	$('#attached_file_name').append(aLink);
	if(file_idVal !=""){
		resSET = file_idVal+','+resSET;
	}
	$('#file_id').val(resSET);
	$("#myModal").modal('hide');  
});
$(document).on('click', '.del-file', function (e) {
	...
	bootbox.confirm(lang['message-delete'], function (result) {
		if (result == true) {
			...
			var farray 		= $('#file_id').val().split(",");
			var fileArray 	= [];
			$.each(farray,function(i){
				if(farray[i] == fid){
				}else{
					fileArray.push(farray[i]);
					//fileArray[] =farray[i];
				}
			});	
			var energy = fileArray.join(",");
			$('#file_id').val(energy);
		}
	});	
