...
@section('content')
	<section class="panel" id="purchases-report">
		<header class="panel-heading">
			<div class="row">
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6"><strong>Vending Sales Report</strong></div>
			</div>
		</header>
	...
	<script type="text/javascript" class="init">
        $(document).ready(function () {
			...
			@if($isSuLevel)
                ...
                ajax: ({
                    type: "GET",
                    url: "{{ url('/vending-sales/json') }}", // json datasource
                    data: {
                        unit_id: '{{ $unitId ?: null }}',
                ...
                buttons: [
                    {
                        ...
                        action: function (e, dt, node, config) {
                            var vendingSalesCount = $('.checkboxs:checked').length;
                            var vendingSalesToDel = vendingSalesCount == 1 ? 'this vending sales record' : 'these vending sales records';
                            var result = confirm("Do you want to delete " + vendingSalesToDel + "?");
                            if (result) {
                                if (vendingSalesCount > 0) {  // at-least one checkbox checked
                                    ...
                                    $.ajax({
                                        type: "get",
                                        url: '{{ url('/vending-sales/delete') }}' + '/' + ids_string,
                                        data: {_token: '{{csrf_token()}}'},
                                        success: function (data) {
                                            var vendingSalesIds = data.split(',');
                                            for (var i = 0; i < vendingSalesCount; i++) {
                                                oTable
                                                    .row($('#tr_' + vendingSalesIds[i]))
                                                    .remove()
                                                    .draw(false);
                                            }
        ...
        $('#example tbody').on('click', 'button.delete', function () {
            var result = confirm("Do you want to delete this vending sales record?");
            if (result) {
                ...
                $.ajax({
                    type: 'get',
                    url: '{{ url('/vending-sales/delete') }}' + '/' + id[1],
                    data: {_token: token},
                    success: function (data) {
                        if (data) {
                            oTable
