...
class UnitClosed extends Model
{
	...
	{
		return self::select('*')->where('unit_id', '=', $unit_id)->where('month', '=', $month)->where('year', '=', $year)->first();
	}

	public static function getUnitClosedByAll($unit_id = 0, $month = 0, $year = 0)
	{
		return self::select('*')->where('unit_all', '=', $unit_id)->where('month', '=', $month)->where('year', '=', $year)->first();
	}
}