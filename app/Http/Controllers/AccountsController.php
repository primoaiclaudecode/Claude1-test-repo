...
class AccountsController extends Controller
{
    ...
    public function statementCheck(Request $request)
    {
        ...
        if ($request->has('submit') && $request->submit == 'Submit') {
            if (Gate::denies('su-user-group') && Gate::denies('admin-user-group')) {
                abort(403, 'Access denied. You need to be SU or admin');
            }
            $stmntChk = 1;
            $uniqueId = '';
            $frozen = '';
        ...
        $fromDate = $request->input('from_date', Cookie::get('statementCheckFromDateCookie', Carbon::now()
            ->subMonth()
            ->format('d-m-Y')));
    ...
    public function bsiReport(Request $request)
    {
        ...
        if ($request->has('submit') && $request->submit == 'Get Report') {
            ...
            foreach ($bsiReportData as $bsirKey => $bsirValue) {
                ...
                $sageAccountNumberData = \DB::table('suppliers')
                    ->select('sage_account_number')
                    ->where('suppliers_id', $supplierIdBsi)
                    ->first();
                $sageAccountNumber = $sageAccountNumberData->sage_account_number;

                $netExtData = \DB::table('nominal_codes')
                    ->select('net_ext', 'nominal_code')
                    ->where('net_ext_ID', $netExtIdBsi)
                    ->first();
                ...
                (
                    [
                        'supplier'            => $supplierBsi,
                        'invoice_date'        => $invoiceDateBsi,
                        'invoice_number'      => $invoiceNumberBsi,
                        'nom_code'            => $nominalCodeBsi,
                        'net_ext'             => $netExtBsi,
                        'purchase_details'    => $purchaseDetailsBsi,
                        'net'                 => $netBsi,
                        'tax_code'            => $taxCodeTitle,
                        'vat'                 => $vatBsi,
                        'gross'               => $grossBsi,
                        'user_id'             => $userId,
                        'sage_account_number' => $sageAccountNumber,
                        'transaction_type'    => $transactionType,
                        'unit_name'           => $unitNameBsi,
                        'id'                  => $purchaseIdBsi,
                        'unit_id'             => $unitIdBsi,
                        'net_ext_ID'          => $netExtIdBsi,
                    ]
                );
            }
    ...
    public function purchasesSalesUnitCloseJson(Request $request)
    {
        ...
        $unitArr = [ 'total_net'             => $totalNet,
                     'total_vat'             => $totalVat,
                     'total_gross'           => $totalGross,
                     'total_sales'           => $totalSales,
                     'closed_unit_error_msg' => $closedUnitErrorMsg ];
