...
var invoiceDate = document.getElementById('sheet_invoice_dates');
...
checkField = [supervisor];

function prepareEventHandlers() {
	document.getElementById('credit_purchase').onsubmit = function() {
		
	// Show confirm dialogue box if user selects less or more then 60 days from today from Invoice Date [ Starts ]
	var d = new Date();
	...
	if($("#invoice_date").datepicker('getDate') < d || ($("#invoice_date").datepicker('getDate') > e)) {
		var c = confirm("Warning, your Invoice Date is more than 60 days from today's date.  Please review the Invoice Date and select \"Confirm\" to accept, or \"Cancel\" to change.");
    	return c;		
	}
	// Show confirm dialogue box if user selects less or more then 60 days from today from Invoice Date [ Ends ]
		
		var i;
		for(i=0; i < checkField.length; i++){
			if ( checkField[i].value == "" ){
				checkField[i].className = "errorfield";
				checkField[i].focus();
				var errorMsg = document.getElementsByClassName('error_message')[i];
				errorMsg.innerHTML = "Field cannot be left blank.";
				return false;
			} else {
				var errorMsg = document.getElementsByClassName('error_message')[i];
				errorMsg.innerHTML = "";
				checkField[i].className = "";
			}
		}

		for(i=0; i<checkNum.length; i++){
			if(isNaN(checkNum[i].value)){
				checkNum[i].className = "errorfield";
				checkNum[i].focus();
				var errorMsg = document.getElementsByClassName('error_num')[i];
				errorMsg.innerHTML = "<br>Field can only contain numbers.";
				return false;
			} else {
				var errorMsg = document.getElementsByClassName('error_num')[i];
				errorMsg.innerHTML = "";
				checkNum[i].className = "";
			}
		}
...
// Implementing Uniqueness check on Invoice number based on Supplier [ Starts ]
function checkInputData() {
	var unit_name_val = $("#unit_name").val();
    var supplier_val = $("#supplier").val();
	var invoice_number_val = $("#invoice_number").val();
	var sheet_id_val = $("#unique_id").val();
    if(sheet_id_val)
    	var data = 'supplier='+supplier_val+'&invoice_number='+invoice_number_val+'&sheet_id='+sheet_id_val;
    else
    	var data = 'supplier='+supplier_val+'&invoice_number='+invoice_number_val;
	...
	} else if(supplier_val == '0') {
		$("#unit_name").removeClass("errorfield");
		$("#unit_name_span.error_message").html("");
		$("#supplier").addClass("errorfield");
		$("#supplier").focus();
		$("#suppliers_span.error_message").html("Please select a Supplier.");
	} else if($("#sheet_invoice_dates").datepicker('getDate') < d || ($("#sheet_invoice_dates").datepicker('getDate') > e)) {
		alert("Invoice Date cannot be in the future or > 1 year in the past.");
		$("#sheet_invoice_dates").focus();
		return false;		
	} else if(invoice_number_val == '') {
		$("#unit_name").removeClass("errorfield");
		$("#unit_name_span.error_message").html("");
		$("#supplier").removeClass("errorfield");
		$("#suppliers_span.error_message").html("");
		$("#invoice_number").addClass("errorfield");
		$("#invoice_number").focus();
		$("#invoice_number_span.error_message").html("Field cannot be left blank.");
		...
		$.ajax({
			...
			success: function(result){
				...
				if(result == "Exists") {  
					...
					//$("#invoice_number").addClass("errorfield");
					$("#invoice_number").focus();
					$("#invoice_number_span.error_message").html("This Invoice Number is already in use.");
				} else {
					//$("#invoice_number").removeClass("errorfield");
					$("#invoice_number_span.error_message").html("");
...
function validate_tax_rate() {
	...
	// check of previous net ext, goods and tax rate [ Starts ]
		if(document.getElementById("net_ext_"+parseInt(rowCount)).value == '') {
...
$(document).ready(function() {
    $("#credit_purchase").on("submit", function () {
...
// Implementing Uniqueness check on Invoice number based on Supplier [ End ]

//
function supplier_reset() {
	var supplier_val = $("#supplier").val();
	if(supplier_val != '0') {
		$("#supplier").removeClass("errorfield");
		$("#suppliers_span.error_message").html("");
...
function addRow(tableID) {
	...
	if(document.getElementById("net_ext_"+parseInt(rowCount)).value == '') {
		//alert('Please select a net ext');
		$("#net_ext_"+parseInt(rowCount)+".error_message").html("Please select a net ext");
		$("#net_ext_"+parseInt(rowCount)).focus();
		return false;
	} else {
		$("#net_ext_"+parseInt(rowCount)+".error_message").html("");
	}
...
function calculations() {
	...
	for(var i = 1; i <= total_rows; i++) {
		...
		//goods_val = parseFloat(goods_val);
		if(goods_val >= 0) {
